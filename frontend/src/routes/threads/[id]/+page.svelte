...
<div class="relative h-screen bg-gray-900 text-white flex flex-col overflow-hidden">
  <IdentityPanel bind:open={panelOpen} />

  <div class="flex-1 flex flex-col min-h-0">
    <header class="flex-none z-30 bg-gray-900 border-b border-gray-700 p-4 flex items-center justify-between shadow-md">
      <button
      ...
      <a href="/events" class="text-indigo-400 hover:text-indigo-300 transition">
        ← Back to Events
      </a>
    </header>
    ...
    <div class="flex-none z-30 bg-gray-900 border-t border-gray-700 p-4 shadow-lg">
      <div class="flex items-center space-x-2">
        <input
          ...
          on:keydown={(e) => e.key === "Enter" && newMessage.trim() && sendMessage()}
        />
        {#if newMessage.trim()}
          <button
