...
*   **Columns:**
    ...
    *   `agent_name` (String, required) - The name of the agent to delete.
    *   `agent_secret` (String, optional) - To help verify ownership if provided.
...
*   **Behavior:**
    *   On submit, it fires a `POST` request to the backend.
...
*   **Action:** Inserts a new row into the `delete_requests` table with `status="pending"`.
...
*   **Dashboard UI (Two Columns or Tabs):**
    *   **Section A: Deletion Requests**
        ...
        *   Shows `agent_name`, `agent_secret` (if provided), and `requested_at`.
        *   **Action Button:** "Approve Deletion" -> Finds all `entries` matching that `agent_name` and soft/hard deletes them, then marks the request as "completed".
...
2.  **`POST /api/compliance/admin/requests/{request_id}/approve`**
    *   Executes the bulk deletion of entries for the associated `agent_name` (and `agent_secret` if provided/matching). Updates the request status to "completed".
...
*   The public `delete-me` endpoint should ideally have basic rate limiting if possible to prevent spamming the database with fake requests.
